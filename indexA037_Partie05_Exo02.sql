...
Exercice 02 - Partie V
Tables utilisées :
lpecom_covid
lpecom_regions
*/

-- Quelle requête utiliser pour afficher toutes les données de vaccination 
-- uniquement pour le 1er avril 2021 avec le nom de la région concernée ?
SELECT 
    c.*, r.name
FROM
    lpecom_covid c
        JOIN
    lpecom_regions r ON (c.id_region = r.id)
WHERE
    jour = '2021-04-01';